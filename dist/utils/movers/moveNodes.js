...
function createGroupBox(graph, groupName) {
...
export function moveNodes(graph, snapTo) {
	...
	function moveGroup() {
		...
		nodeGroupArray.forEach((node, index) => {
			...
			if (!groupBox) {
				moveElement(initialPosition, delta, position, node);
			} else {
				...
				moveElementWithBounds(initialPosition, delta, position, bounds, node);
			}
		});
		if (get(tracking)) {
...
export function moveElement(initialPosition, delta, position, node) {
	const newPosition = {
...
export function moveElementWithBounds(initialPosition, delta, position, bounds, node) {
	const newPosition = {
