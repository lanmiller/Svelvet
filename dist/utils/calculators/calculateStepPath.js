...
export function calculateSmartStepPath(source, target, options = {}) {
    // Значения по умолчанию
    const { cornerRadius = 8, stepBuffer = 40, bendingRules = 'smart', preferredDirection = 'auto', avoidOverlap = true, maxBends = 4, nodeWidth = 200, nodeHeight = 100, gridSize = 25, obstacleAvoidance = false } = options;
    const steps = [];
...
function calculateMinimalBendPath(source, target, buffer, preferredDirection) {
    const steps = [];
    const deltaX = target.x - source.x;
    const deltaY = target.y - source.y;
...
function calculateDirectPath(source, target, buffer) {
    const steps = [];
    const deltaX = target.x - source.x;
    const deltaY = target.y - source.y;
    // Минимальные отступы
...
function calculateSmartPath(source, target, buffer, preferredDirection, maxBends, avoidOverlap) {
    const steps = [];
    const deltaX = target.x - source.x;
    const deltaY = target.y - source.y;
    const absX = Math.abs(deltaX);
