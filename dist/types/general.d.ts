...
export type StrictDimenionValue = number | PixelValue | RemValue | EmValue | PercentValue | VwValue | VhValue;
export type XorY = 'x' | 'y';
type OptionalSpace = ' ' | '';
export type CSSColorString = NamedColors | `#${string}` | RGBString | `rgba(${OptionalSpace}${number},${OptionalSpace}${number},${OptionalSpace}${number},${OptionalSpace}${number | string}${OptionalSpace})` | HSLString | `hsla(${OptionalSpace}${number},${OptionalSpace}${number}%,${OptionalSpace}${number}%,${OptionalSpace}${number | string}${OptionalSpace})`;
export type RGBString = `rgb(${OptionalSpace}${number},${OptionalSpace}${number},${OptionalSpace}${number}${OptionalSpace})`;
export type HSLString = `hsl(${OptionalSpace}${number},${OptionalSpace}${number}%,${OptionalSpace}${number}%${OptionalSpace})`;
declare const arrowTuple: readonly ["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"];
export type Arrow = (typeof arrowTuple)[number];
export declare const isArrow: (key: string) => key is "ArrowLeft" | "ArrowRight" | "ArrowUp" | "ArrowDown";
